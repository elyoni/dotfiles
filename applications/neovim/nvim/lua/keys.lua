...
vim.keymap.set("n", "<F5>", "<cmd>r! date '+\\%Y/\\%m/\\%d \\%H:\\%M:\\%S'<CR>",
    with_desc("Insert current date (normal mode)"))
vim.keymap.set("i", "<F5>", "<C-r>=trim(system(\"date '+%Y/%m/%d %H:%M:%S'\"))<CR>",
    with_desc("Insert current date (insert mode)"))
